...
SELECT COUNT(DISTINCT Composer) as Composer_number
FROM tracks;
...
SELECT name, MIN(Milliseconds) as track_has_min_duration
FROM tracks;

SELECT name, MAX(Milliseconds) as track_has_max_duration
FROM tracks;

/* WRITE A QUERY THAT RETURNS THE TRACKS HAVING DURATION LESS 
THAN THE AVERAGE DURATION */

SELECT Milliseconds, AVG(Milliseconds)
FROM tracks;
...
SELECT Composer,
COUNT(Composer) as total_number_composer_track
FROM tracks 
GROUP BY Composer;

SELECT Composer,
COUNT(*) as total_number_composer_track
FROM tracks 
GROUP BY Composer;
...
SELECT artists.name, artists.ArtistId, albums.AlbumId, albums.Title
FROM artists 
